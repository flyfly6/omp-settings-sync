...
export default function gitSyncExtension(pi) {
    ...
    pi.on("session_start", async (_event, ctx) => {
        ...
        if (!isSubagentChild() && (await isSyncableRepo())) {
            try {
                ...
                const msg = error instanceof Error ? error.message : String(error);
                if (ctx.hasUI && ctx.ui)
                    ctx.ui.notify(`omp-sync skipped: ${msg}`, "warning");
            }
        }
        // 2. Schedule 1-minute background sync interval to sync whenever changes occur
        const scheduleInterval = typeof ctx.setInterval === "function"
            ? (fn, ms) => ctx.setInterval(fn, ms)
            : (fn, ms) => {
    ...
    pi.on("session_shutdown", async (event, ctx) => {
        const reason = typeof event === "object" && event && "reason" in event && typeof event.reason === "string"
            ? event.reason
            : undefined;
        if (reason === "reload" || isSubagentChild())
            return;
